...
$this->title = Yii::t('app', 'Send Milestone for QA: ' .$oModel['name']);
...
<div class="pages-index">
  ...
  <?= $form->field($oModel,'qa_user_id')->dropDownList($qaUsersArr,[
        'prompt' => Yii::t('app', '-Choose QA Person-'),'class' => 'form-control qa_required'])->label('Choose QA person'); ?>
         <?= $form->field($oModel,'notes')->textArea(['rows' => 7, 'cols' => 50,'class'=>'form-control notes_required'])->label('Add Note'); ?>
    	
     		<div class="form-group form-actions">
     			<input type='hidden' name="mid" value="<?= $_GET['mid'] ?>">
           		 <?= Html::submitButton('Send', ['class' => 'btn btn-success send_to_qa']) ?>
...
$(document).ready(function(){
    ...
    // START OF CUSTOM MESSAGES AND VALIDATION FOR create_leave_form//
     $.validator.addMethod("qa_required", function(value, element) {
      //If false, the validation fails and the message below is displayed
        var qa_user = value;
        return qa_user != '';
      }, "Choose at least a QA person");
